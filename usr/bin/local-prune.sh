...
git fetch -p || printf "This message means that you haven't specified your origin/upstream repo! Do that, then run this command again."; exit
git pull
git branch -vv | awk '/: gone]/{print $1}' | xargs git branch -d || printf "This means that there are no deleted remote branches which local branches were tracking."; exit
